...
def _ensure_page_state(refs: Dict[str, Any]) -> None:
    ...
    st.session_state.setdefault("m1_design_standard", refs["standards_list"][0] if refs["standards_list"] else "")
...
def _apply_defaults_to_widgets(n: int, defaults: List[Dict[str, Any]]) -> None:
    for i in range(1, n + 1):
        ...
        st.session_state[f"m1_bs_{i}"] = d.get("beam_section", "")
        st.session_state[f"m1_bg_{i}"] = d.get("beam_grade", "")
        st.session_state[f"m1_cs_{i}"] = d.get("column_section", "")
        st.session_state[f"m1_cg_{i}"] = d.get("column_grade", "")
...
def _apply_bulk_module1(num_storeys: int, refs: Dict[str, Any], start: int, end: int) -> None:
    ...
    values = {
        ...
        "beam_section": st.session_state.get("m1_bulk_beam_section", refs["section_names"][0]),
        "beam_grade": st.session_state.get("m1_bulk_beam_grade", refs["grades"][0]),
        "column_section": st.session_state.get("m1_bulk_column_section", refs["section_names"][0]),
        "column_grade": st.session_state.get("m1_bulk_column_grade", refs["grades"][0]),
    }
...
def _render_bulk_tools(num_storeys: int, refs: Dict[str, Any]) -> None:
    with st.expander("Fast Input Tools", expanded=True):
        ...
        r4.selectbox("Beam section value", refs["section_names"], key="m1_bulk_beam_section")
        r5.selectbox("Beam grade value", refs["grades"], key="m1_bulk_beam_grade")
        r6.selectbox("Column section value", refs["section_names"], key="m1_bulk_column_section")
        r7.selectbox("Column grade value", refs["grades"], key="m1_bulk_column_grade")
        ...
        if a3.button("Apply to range", key="m1_apply_range"):
            _apply_bulk_module1(num_storeys, refs, int(st.session_state["m1_bulk_from"]), int(st.session_state["m1_bulk_to"]))
            st.rerun()
...
def _build_rows(n: int, refs: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for i in range(1, n + 1):
        ...
        if c4.button("Copy above", key=f"m1_copy_above_{i}", disabled=(i == 1)):
            ...
            st.session_state[f"m1_bs_{i}"] = st.session_state.get(f"m1_bs_{i-1}", refs["section_names"][0])
            st.session_state[f"m1_bg_{i}"] = st.session_state.get(f"m1_bg_{i-1}", refs["grades"][0])
            st.session_state[f"m1_cs_{i}"] = st.session_state.get(f"m1_cs_{i-1}", refs["section_names"][0])
            st.session_state[f"m1_cg_{i}"] = st.session_state.get(f"m1_cg_{i-1}", refs["grades"][0])
            st.rerun()

        c5, c6, c7, c8 = st.columns(4)
        section_names = refs["section_names"]
        grades = refs["grades"]

        beam_section = c5.selectbox(
            f"Beam Section - S{i}",
            section_names,
            index=section_names.index(st.session_state.get(f"m1_bs_{i}", section_names[0])) if st.session_state.get(f"m1_bs_{i}") in section_names else 0,
            key=f"m1_bs_{i}",
        )
        beam_grade = c6.selectbox(
            f"Beam Grade - S{i}",
            grades,
            index=grades.index(st.session_state.get(f"m1_bg_{i}", grades[0])) if st.session_state.get(f"m1_bg_{i}") in grades else 0,
            key=f"m1_bg_{i}",
        )
        column_section = c7.selectbox(
            f"Column Section - S{i}",
            section_names,
            index=section_names.index(st.session_state.get(f"m1_cs_{i}", section_names[0])) if st.session_state.get(f"m1_cs_{i}") in section_names else 0,
            key=f"m1_cs_{i}",
        )
        column_grade = c8.selectbox(
            f"Column Grade - S{i}",
            grades,
            index=grades.index(st.session_state.get(f"m1_cg_{i}", grades[0])) if st.session_state.get(f"m1_cg_{i}") in grades else 0,
            key=f"m1_cg_{i}",
        )
        rows.append({
            "storey": i,
...
def _render_export_buttons(results: Dict[str, Any]) -> None:
    with TemporaryDirectory() as tmpdir:
        ...
        st.download_button("Download JSON", data=json_path.read_bytes(), file_name="module1_results.json", mime="application/json")
        st.download_button("Download Excel", data=xlsx_path.read_bytes(), file_name="module1_results.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
def render_module1_page(data_dir: Path, refs: Dict[str, Any]) -> None:
    ...
    if st.button("Load sample Module 1 input"):
        ...
        if sample.design_standard in refs["standards_list"]:
            st.session_state["m1_design_standard"] = sample.design_standard
        defaults = storey_rows_module1_from_frame(sample)
    ...
    if defaults is None:
        ...
        for i in range(1, min(len(defaults), num_storeys) + 1):
            current_rows.append({
                ...
                "beam_section": st.session_state.get(f"m1_bs_{i}", defaults[i-1].get("beam_section", "")),
                "beam_grade": st.session_state.get(f"m1_bg_{i}", defaults[i-1].get("beam_grade", "")),
                "column_section": st.session_state.get(f"m1_cs_{i}", defaults[i-1].get("column_section", "")),
                "column_grade": st.session_state.get(f"m1_cg_{i}", defaults[i-1].get("column_grade", "")),
            })
    ...
    if st.button("Run Module 1", type="primary"):
        ...
        try:
            ...
            results = run_module1(frame, refs["sections"], refs["materials"], refs["standards"])
    ...
    if results:
        ...
        max_u = max(max(r["beam_utilization_ratio"], r["column_utilization_ratio"]) for r in results["results_by_storey"])
