...
def _default_rows(n: int) -> List[Dict[str, Any]]:
    return [
        {"storey": i, "height_m": 3.0, "dead_load_kn_per_m": 5.0, "live_load_kn_per_m": 3.0}
...
def _ensure_page_state(refs: Dict[str, Any]) -> None:
    ...
    st.session_state.setdefault("m2_design_standard", refs["standards_list"][0] if refs["standards_list"] else "")
...
def _render_bulk_tools(num_storeys: int) -> None:
    with st.expander("Fast Input Tools", expanded=True):
        st.caption("Quickly fill repeated heights and loads for the competition, then adjust only the exceptions.")
        ...
        r2.number_input("Dead load value (kN/m)", min_value=0.0, value=5.0, key="m2_bulk_dead")
        r3.number_input("Live load value (kN/m)", min_value=0.0, value=3.0, key="m2_bulk_live")
        a1, a2, a3, a4 = st.columns([1, 1, 1, 2])
        a1.number_input("From storey", min_value=1, max_value=num_storeys, value=1, step=1, key="m2_bulk_from")
        a2.number_input("To storey", min_value=1, max_value=num_storeys, value=num_storeys, step=1, key="m2_bulk_to")
        if a3.button("Apply to range", key="m2_apply_range"):
            _apply_bulk_module2(num_storeys, int(st.session_state["m2_bulk_from"]), int(st.session_state["m2_bulk_to"]))
            st.rerun()
...
def _build_rows(n: int) -> List[Dict[str, Any]]:
    ...
    for i in range(1, n + 1):
        ...
        if cols[4].button("Copy above", key=f"m2_copy_above_{i}", disabled=(i == 1)):
            ...
            st.rerun()
        rows.append({"storey": i, "height_m": height, "dead_load_kn_per_m": dead, "live_load_kn_per_m": live})
    return rows
...
def _render_export_buttons(results: Dict[str, Any]) -> None:
    with TemporaryDirectory() as tmpdir:
        ...
        st.download_button("Download JSON", data=json_path.read_bytes(), file_name="module2_results.json", mime="application/json")
        st.download_button("Download Excel", data=xlsx_path.read_bytes(), file_name="module2_results.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
def render_module2_page(data_dir: Path, refs: Dict[str, Any]) -> None:
    ...
    if defaults is None:
        ...
        for i in range(1, min(len(defaults), num_storeys) + 1):
            current_rows.append({
                "storey": i,
                "height_m": float(st.session_state.get(f"m2_h_{i}", defaults[i-1].get("height_m", 3.0))),
                "dead_load_kn_per_m": float(st.session_state.get(f"m2_d_{i}", defaults[i-1].get("dead_load_kn_per_m", 5.0))),
                "live_load_kn_per_m": float(st.session_state.get(f"m2_l_{i}", defaults[i-1].get("live_load_kn_per_m", 3.0))),
            })
    ...
    umin = util1.number_input("Minimum Utilization", min_value=0.0, max_value=10.0, value=float(default_constraints.utilization_min) if default_constraints else 0.5)
    umax = util2.number_input("Maximum Utilization", min_value=0.0, max_value=10.0, value=float(default_constraints.utilization_max) if default_constraints else 0.7)
    ...
    beam_shapes = shape1.multiselect("Allowed Beam Shapes", SHAPE_ORDER, default=default_beam_shapes)
    column_shapes = shape2.multiselect("Allowed Column Shapes", SHAPE_ORDER, default=default_col_shapes)

    default_allowed_grades = default_constraints.allowed_steel_grades if default_constraints and default_constraints.allowed_steel_grades else refs["grades"]
    allowed_grades = st.multiselect("Allowed Steel Grades", refs["grades"], default=default_allowed_grades)
    ...
    beam_group_text = group1.text_area("Beam Same Groups", value=_group_text(beam_groups), help="One group per line. Examples: 2-4 or 1,3,5")
    column_group_text = group2.text_area("Column Same Groups", value=_group_text(col_groups), help="One group per line. Examples: 1-4 or 2,5")

    class1, class2 = st.columns(2)
    beam_class_dict = default_constraints.beam_max_section_class_by_storey if default_constraints else {}
    column_class_dict = default_constraints.column_max_section_class_by_storey if default_constraints else {"1": 2, "2": 2, "3": 2}
    ...
    if st.button("Run Module 2", type="primary"):
        try:
            ...
            results = optimize_module2(frame, refs["sections"], refs["materials"], refs["standards"])
        ...
        if results["feasible"]:
            ...
            for report in results.get("infeasibility_reports", []):
                with st.expander(f"{report['member_type'].title()} group {report['group']}"):
                    st.write(report.get("message", "No message available."))
